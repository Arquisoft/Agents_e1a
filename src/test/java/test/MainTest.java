...
public class MainTest {
	...
	public void t2domainModelToString() {
		...
		assertEquals(agent1.toString(),
				"Agent [id=" + agent1.getId() + ", nombre=" + agent1.getNombre() + ", location=" + ", email="
						+ agent1.getEmail() + ", identifier=" + agent1.getIdentifier() + ", password="
						+ agent1.getPassword() + ", kind=" + agent1.getKind() + "]");
	}
	...
	public void t6incorrectLogin() {
		...
		String userURI = base.toString() + "/agent";
		String incorrectPassword = "{\"reason\": \"Login incorrect, check params\"}";
		...
		response = template.postForEntity(userURI, new PeticionInfoREST("usuarioA6-PK27", "password", "Person"),
				String.class);
		assertThat(response.getBody(), equalTo(incorrectPassword));

		response = template.postForEntity(userURI, new PeticionInfoREST("usuarioRace", "23456", "Sensor"),
				String.class);
		assertThat(response.getBody(), equalTo(incorrectPassword));
	}
	...
	public void t8emptyKind() {
		...
		response = template.postForEntity(userURI, new PeticionInfoREST("usuarioA6-PK27", "paswword", ""),
				String.class);
		assertThat(response.getBody(), equalTo(emptyPassword));
	}
	...
	public void emailChangeCorrect() {
		...
		response = template.postForEntity(userURI, new PeticionChangeEmailREST("usuarioJuan", "j@hotmail.com"),
				String.class);
		...
		response = template.postForEntity(userURI, new PeticionChangeEmailREST("usuarioRace", "race@gmail.com"),
				String.class);
		assertThat(response.getBody(), equalTo(correctChange));

		correctChange = "{\"agent\":\"juan@uniovi.es\",\"message\":\"email actualizado correctamente\"}";
		response = template.postForEntity(userURI, new PeticionChangeEmailREST("usuarioJuan", "juan@uniovi.es"),
				String.class);
		assertThat(response.getBody(), equalTo(correctChange));
	}
	...
	public void emailChangeCorrectXML() {
		...
		String correctChange = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>" + "<ChangeInfoResponse>"
				+ "<agent>avisos@race.es</agent>" + "<message>email actualizado correctamente</message>"
				+ "</ChangeInfoResponse>";
	...
	// TODO hacer cambios en el controlador antes @Test
	...
	public void t13invalidEmailChange() {
		...
		response = template.postForEntity(userURI, new PeticionChangeEmailREST("usuarioA6-PK27", "paco@"),
				String.class);
		assertThat(response.getBody(), equalTo(wrongEmailStyle));
	...
	public void t15emailChangeUserNotFound() {
		...
		response = template.postForEntity(userURI, new PeticionChangeEmailREST("usuarioJuanito", "juan@hotmail.com"),
				String.class);
		assertThat(response.getBody(), equalTo(userNotFound));

		response = template.postForEntity(userURI, new PeticionChangeEmailREST("user", "pepe@gmail.com"), String.class);
		assertThat(response.getBody(), equalTo(userNotFound));

		response = template.postForEntity(userURI, new PeticionChangeEmailREST("usarioInexistente", "pa@hotmail.com"),
				String.class);
		assertThat(response.getBody(), equalTo(userNotFound));
	}
	...
	public void t16sameEmailErrorChange() {
		...
		response = template.postForEntity(userURI, new PeticionChangeEmailREST("usuarioA6-PK27", "tecnico@copinsa.es"),
				String.class);
		assertThat(response.getBody(), equalTo(sameEmail));
	}
	...
	public void correctPasswordChange() {
		...
		response = template.postForEntity(userURI, new PeticionChangePasswordREST("usuarioJuan", "password", "djfhr"),
				String.class);
	...
	public void correctPasswordChangeXML() {
		...
		String userURI = base.toString() + "/changePassword";
		String correctChange = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>" + "<ChangeInfoResponse>"
				+ "<agent>usuarioJuan</agent>" + "<message>contrase̱a actualizada correctamente</message>"
				+ "</ChangeInfoResponse>";
		...
		response = template.postForEntity(userURI, new PeticionChangePasswordREST("usuarioJuan", "djfhr", "password"),
				String.class);
		assertThat(response.getBody(), equalTo(correctChange));
	}
	...
	public void t19passwordRequiredPasswordChange() {
		...
		response = template.postForEntity(userURI, new PeticionChangePasswordREST("usuarioJuan", "", ""), String.class);
	...
	public void t21samePasswordChange() {
		...
		response = template.postForEntity(userURI, new PeticionChangePasswordREST("usuarioJuan", "djvhrhc", "djvhrhc"),
				String.class);
		assertThat(response.getBody(), equalTo(passwordRequired));
	...
	public void t22notFoundAgentPasswordChange() {
		...
		response = template.postForEntity(userURI, new PeticionChangePasswordREST("UsuarioJuanito", "djfhr", "djfhrtt"),
				String.class);
		assertThat(response.getBody(), equalTo(userNotFound));

		response = template.postForEntity(userURI, new PeticionChangePasswordREST("user", "djvhrhc", "tt"),
				String.class);
		assertThat(response.getBody(), equalTo(userNotFound));
	...
	public void t23notSamePasswordChange() {
		...
		response = template.postForEntity(userURI, new PeticionChangePasswordREST("usuarioRace", "djvhrhc", "djvhrhc"),
				String.class);
		assertThat(response.getBody(), equalTo(passwordIncorrect));
